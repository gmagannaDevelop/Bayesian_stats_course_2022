...
- compute complex integrals $\int_a^b h(x)dx$ where we know what $h(x)$ looks like mathematically but not its integral (you can do thus numerically too, but not when $x$ is of dimension 10 or 100). \pause $\mathbb{E}(f(X)) = \int f(x) p(x) dx$. \pause

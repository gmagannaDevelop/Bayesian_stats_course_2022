...
#here::here()
library(here)
dat<-read.csv(here("Bayesian_stats_course_2022/session_4/crestedTit.csv"))
#str(dat)
dat$X<-NULL
# head(dat)
...
str(bdatal <- list(
  LC=LC[sel,], M = newM, T = ncol(LC[sel,]), 
  yr = (year - mean(year))/sd(year), # sans standardiser / sd(year), c'est plus facile a interpreter
  elev = elev.sc[sel,], forest = forest.sc[sel,], 
  date = date.sc[sel,], dur = dur.sc[sel,],
...
cat(file = "model_lmm4.txt","
    ...
    # Random effects (linear declaration)
    ## Site 
      sd.site ~ dunif(0, 5)
    ...
    for (i in 1:M){
      for(t in 1:T){
        lambda[i,t] <- mu + site[i] + year[t] + gamma * yr[t] +
                        theta[1] * elev[i] + theta[2] * elev[i]^2 +
                        theta[3] * forest[i] + theta[4] * date[i, t] +
                        theta[5] * date[i, t]^2 + theta[6] * dur[i, t] +
                        theta[7] * twosurveys[i]
...
# MCMC settings
na <- 1000 ; ni <- 15000; nt <- 10 ; nb <- 5000 ; nc <- 5 # also core number for parallel computation with jagsUI
...
effect_date = function(x){return(out_lmm4$BUGSoutput$mean$theta[4]*x+out_lmm4$BUGSoutput$mean$theta[5]*x^2)}

curve(effect_elev,from=min(elev.sc),to=max(elev.sc))
curve(effect_date,from=min(date.sc),to=max(date.sc))
...
* date has likely negative effect
* duration has a positive effect
* two or three surveys -> more birds if three surveys but minor effect
...
# MCMC settings
na <- 1000 ; ni <- 15000; nt <- 10 ; nb <- 5000 ; nc <- 5 # core number for parallel computation
